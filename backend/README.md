...
```bash
python server.py
# בהרצה ראשונה יוריד את המודל (כמה מאות MB ל-small)
```

ברירת המחדל: **CPU + מודל `small` + INT8** — עובד על כל מחשב, latency גבוה יותר.

#### מעבר ל-GPU + מודל עברי מלא (איכות מיטבית)
דורש NVIDIA GPU עם CUDA. הגדר משתני סביבה לפני ההרצה:
```bash
# Windows PowerShell:
$env:MHS_MODEL  = "ivrit-ai/whisper-large-v3-turbo-ct2"
$env:MHS_DEVICE = "cuda"
$env:MHS_COMPUTE= "float16"
python server.py
```

| משתנה | ברירת מחדל | אפשרויות |
|-------|-----------|----------|
| `MHS_MODEL` | `small` | כל מודל faster-whisper, למשל `ivrit-ai/whisper-large-v3-turbo-ct2` |
| `MHS_DEVICE` | `cpu` | `cuda` |
